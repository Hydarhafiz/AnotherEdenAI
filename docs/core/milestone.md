...
- Stabilize C3 review sequencing before batch credit: deterministically diagnose every listed C3 family for positive and negative/cross-family fixture coverage; generate the targeted seed artifact only from source-backed parsed facts; and stop with a named coverage gap when a required family has no usable candidate. A gap requires one human-supplied canonical character or sidekick wiki page, exact fact name, and intended atomic capability before it can be added as a fixture; no capability may be inferred, silently skipped, or substituted from a general mechanics page.
...
- The seed generator emits a deterministic, auditable targeted artifact containing one source-backed positive fixture and one negative or cross-family fixture for every uncovered listed C3 family. It reports any unavailable family by capability/rule and accepts no partial seed as complete.
