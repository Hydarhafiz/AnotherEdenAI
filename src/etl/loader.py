    ...
    boss_data = [
        {
            ...
            "hp": r.hp,
            "weak": r.weak,
            "weak_state": r.affinity_state["weak"],
            "resist": r.resist,
            "resist_state": r.affinity_state["resist"],
            "null": r.null,
...
SET s.canonical_id = row.canonical_id,
    ...
    s.source_url = row.source_url,
    s.section_anchor = row.section_anchor,
    s.section_end_anchor = row.section_end_anchor,
    s.source_section = row.source_section,
    s.section_bounded = row.section_bounded,
    s.provenance = row.provenance,
    s.mechanics_evidence = row.mechanics_evidence,
    s.citation_url = row.citation_url,
    s.difficulty_tier = row.difficulty_tier,
    s.cohort = row.cohort,
    s.level = row.level,
    s.hp = row.hp,
    s.weak = row.weak,
    s.weak_state = row.weak_state,
    s.resist = row.resist,
    s.resist_state = row.resist_state,
    s.null = row.null,
    s.null_state = row.null_state,
    s.absorb = row.absorb,
    s.absorb_state = row.absorb_state,
    s.affinity_evidence = row.affinity_evidence,
    s.affinity_observations = row.affinity_observations,
    s.characteristics = row.characteristics,
    s.mechanic_tags = row.mechanic_tags,
    s.mechanics_text = row.mechanics_text,
    s.support_status = row.support_status,
